...
# TODO: support exercise of stock options
class Asset::Option < Asset::Base
  def self.costs(transaction)
    ...
    if transaction.daytrade?
      ...
      items['Emolumentos/Negociação (0,0130%)'] = (transaction.value * 0.00013).floor(2)
      items['Liquidação (0,0180%)'] = (transaction.value * 0.00018).floor(2)
      items['Registro (0,0140%)'] = (transaction.value * 0.00014).floor(2)
    else
      items['Corretagem'] = BigDecimal.new('1.99')
      items['Corretagem - ISS (5%)'] = (BigDecimal.new('1.99') * 0.05).floor(2)
      items['Emolumentos/Negociação (0,0370%)'] = (transaction.value * 0.00037).floor(2)
      items['Liquidação (0,0275%)'] = (transaction.value * 0.000275).floor(2)
      items['Registro (0,0695%)'] = (transaction.value * 0.000695).floor(2)
    end
    items
  end
  ...
  def self.process(transaction)
    ...
    if holding.blank?
      ...
      Asset::Option.calculate_taxes(transaction)
  ...
  def self.calculate_taxes(transaction)
    ...
    aliquot = Asset::Option.tax_aliquot(transaction)
