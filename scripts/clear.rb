...
def near?(value1, value2, max=0.2)
...
def node_below(page, label)
  ...
  return '' if node.nil?
  page.find_all { |n| near?(node['left'], n['left'], 3) && n['top'] >= node['bottom'] }.sort_by { |n| n['top'] }.first
end
...
def process_pdf_path(path)
  ...
  pages.each_with_index do |page, i|
    ...
    headers = page.find_all { |n| near?(node['top'], n['top'], 1) }.sort_by { |n| n['left'] }
    ...
    page.each do |node|
      if node['value'].match(/1\-BOVESPA/i)
        row = page.find_all { |n| near?(node['top'], n['top'], 1) && n['left'] >= node['left'] }.sort_by { |n| n['left'] }
